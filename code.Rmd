...
The other two variables however, have missing values representing the entirity of the values available for the `r format(cc_uv_missing_values.dates, format = "%dst of %B of %Y")` (`r uvIndex_missing_values` out of `r values_cc_uv_missing_values.dates` values).
...
The two data imputation functions which might be able to produce the best outcome are the K-nearest neighbours and missForest data imputation functions.
...
features <- c("day_numeric", "dayOfWeek", "dayOfYear", "isUkHoliday", "temperatureMax", "temperatureMin")

#daily_dataset.filtered.noNa$consumption_imputed <- knn(daily_dataset.filtered.noNa[, features], daily_dataset.filtered.noNa$consumption, k = 5)

features <- c("day_numeric", "consumption", "dayOfWeek", "dayOfYear", "isUkHoliday", "temperatureMax", "temperatureMin")

#daily_dataset.filtered.noNa$uvIndex_imputed <- knn(daily_dataset.filtered.noNa[, features], daily_dataset.filtered.noNa$uvIndex, k = 5)
#daily_dataset.filtered.noNa$cloudCover_imputed <- knn(daily_dataset.filtered.noNa[, features], daily_dataset.filtered.noNa$cloudCover, k = 5)
```
...
ts_consumption.year <- ts(na.omit(daily_dataset.consumption$x), frequency = 365)
ts_consumption.year.decom <- decompose(ts_consumption.year, type="additive")

plot( ts_consumption.year.decom )
plot( ts_consumption.year.decom$x ) + lines(ts_consumption.year.decom$seasonal + ts_consumption.year.decom$trend, col="red")

ts_consumption.year <- ts(na.omit(daily_dataset.consumption[daily_dataset.consumption$day >= "2012-01-01" & daily_dataset.consumption$day <= "2013-12-31",]$x), frequency = 360)
ts_consumption.year.decom <- decompose(ts_consumption.year, type="additive")


mean <- mean(daily_dataset.merged$consumption, na.rm=TRUE)

ts_consumption.year.decom$trend_noNa <- ts_consumption.year.decom$trend
ts_consumption.year.decom$trend_noNa[is.na(ts_consumption.year.decom$trend_noNa)] <- mean
ts_consumption.year.decom$random_noNa <- ts_consumption.year.decom$random
ts_consumption.year.decom$random_noNa[is.na(ts_consumption.year.decom$random)] <- 0

plot( ts_consumption.year.decom )
plot( ts_consumption.year.decom$x ) + lines(ts_consumption.year.decom$seasonal + ts_consumption.year.decom$trend, col="red") + lines(ts_consumption.year.decom$seasonal + ts_consumption.year.decom$trend_noNa + ts_consumption.year.decom$random_noNa, col="red")

```
There is an immediately noticeable seasonal components with yearly frequency. There also is a considerable amount of variance related to the randomicity of the weather conditions mostly observed in specific periods of the year.