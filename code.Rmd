...
for(lcl in LCLid.list) {
  ...
  readingsCount <- count(daily_dataset.filtered[daily_dataset.filtered$LCLid == lcl, ]$LCLid)$freq
  if(readingsCount <= 100){
    # if the number of values is not sufficient we remove the client
    daily_dataset.filtered <- daily_dataset.filtered[!(daily_dataset.filtered$LCLid == lcl), ]
  }else{
